...
> 版本：Phase 1 / M1.5 + Phase 2 / M2.2-D
>
> 契约来源：当前 Go、Python 和 Vue 代码（2026-09-15 核对）。
>
> 状态：接口字段和错误码已按源码整理至 M2.2-D；聊天前端及自动化回归已完成，结果见 [M2.2 验收记录](./M2.2验收记录.md)，真实 Docker 服务和双账号浏览器联调尚未执行。
...
- [ ] Vue 聊天页面已实现乐观发送、失败重试、未读、刷新恢复和删好友后历史只读；仍需在真实双账号浏览器环境完成本项验收。
